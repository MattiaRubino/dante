...
def _activate_runtime_acl() -> None:
    statements = (
        "GRANT INSERT (recovery_restriction_code, "
        ...
        "GRANT SELECT, INSERT, DELETE ON TABLE dante.external_auth_transaction TO dante_runtime",
        "GRANT UPDATE (claimed_at) ON TABLE dante.external_auth_transaction TO dante_runtime",
        ...
        "GRANT SELECT, INSERT, DELETE ON TABLE dante.external_link_challenge TO dante_runtime",
        "GRANT SELECT, INSERT, DELETE ON TABLE dante.external_signup_challenge TO dante_runtime",
        ...
        "GRANT SELECT, INSERT, DELETE ON TABLE dante.account_profile_bootstrap TO dante_runtime",
        ...
        "GRANT SELECT, INSERT, DELETE ON TABLE dante.webauthn_challenge TO dante_runtime",
...
def upgrade() -> None:
    ...
    op.create_check_constraint(
        op.f("ck_email_identity_recovery_restriction_code"),
        "email_identity",
    ...
    op.create_check_constraint(
        op.f("ck_email_identity_recovery_restriction_observed"),
        "email_identity",
        "recovery_restriction_code IS NULL OR recovery_restriction_observed_at IS NOT NULL",
        schema=_SCHEMA,
    )
    op.create_check_constraint(
        op.f("ck_email_identity_recovery_restriction_chronology"),
        "email_identity",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_identity_provider_email"),
        ),
        sa.CheckConstraint(
            "status_code IN ('active','revoked')",
            name=op.f("ck_external_identity_status_code"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_identity_chronology"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_identity_revocation"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_auth_transaction_uuidv7"),
        ),
        sa.CheckConstraint(
            "provider_code IN ('google','apple')",
            name=op.f("ck_external_auth_transaction_provider_code"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_auth_transaction_provider_issuer"),
        ),
        sa.CheckConstraint(
            "purpose_code IN ('sign_in','link','reauthenticate')",
            name=op.f("ck_external_auth_transaction_purpose_code"),
        ),
        sa.CheckConstraint(
            "return_target_code IN ('access','security')",
            name=op.f("ck_external_auth_transaction_return_target_code"),
        ),
        sa.CheckConstraint(
            "octet_length(state_verifier)=32",
            name=op.f("ck_external_auth_transaction_state_verifier_length"),
        ),
        sa.CheckConstraint(
            "octet_length(nonce_verifier)=32",
            name=op.f("ck_external_auth_transaction_nonce_verifier_length"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_auth_transaction_session_binding"),
        ),
        sa.CheckConstraint(
            "isfinite(created_at) AND isfinite(expires_at) "
            "AND expires_at>created_at "
            "AND expires_at<=created_at+interval '15 minutes'",
            name=op.f("ck_external_auth_transaction_chronology"),
        ),
        sa.CheckConstraint(
            "claimed_at IS NULL OR (isfinite(claimed_at) "
            "AND claimed_at>=created_at AND claimed_at<=expires_at)",
            name=op.f("ck_external_auth_transaction_claimed_at"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_apple_auth_grant_uuidv7"),
        ),
        sa.CheckConstraint(
            "issuer='https://appleid.apple.com'",
            name=op.f("ck_apple_auth_grant_issuer"),
        ),
        sa.CheckConstraint(
            "subject=btrim(subject) AND subject<>'' AND char_length(subject)<=255",
            name=op.f("ck_apple_auth_grant_subject"),
        ),
        sa.CheckConstraint(
            "client_id=btrim(client_id) AND client_id<>'' AND char_length(client_id)<=255",
            name=op.f("ck_apple_auth_grant_client_id"),
        ),
        sa.CheckConstraint(
            "status_code IN ('pending','active','revocation_pending','revoked')",
            name=op.f("ck_apple_auth_grant_status_code"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_apple_auth_grant_chronology"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_apple_auth_grant_ciphertext"),
        ),
        sa.CheckConstraint(
            "refresh_token_nonce IS NULL OR octet_length(refresh_token_nonce)=12",
            name=op.f("ck_apple_auth_grant_nonce"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_apple_auth_grant_encryption_key_id"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_apple_auth_grant_state"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_link_challenge_uuidv7"),
        ),
        sa.CheckConstraint(
            "provider_code IN ('google','apple')",
            name=op.f("ck_external_link_challenge_provider_code"),
        ),
        sa.CheckConstraint(
            "(provider_code='google' AND issuer='https://accounts.google.com') OR "
            "(provider_code='apple' AND issuer='https://appleid.apple.com')",
            name=op.f("ck_external_link_challenge_provider_issuer"),
        ),
        sa.CheckConstraint(
            "subject=btrim(subject) AND subject<>'' AND char_length(subject)<=255",
            name=op.f("ck_external_link_challenge_subject"),
        ),
        sa.CheckConstraint(
            "(provider_email_address IS NULL AND provider_email_private IS NULL) OR "
            "(provider_email_address IS NOT NULL "
            ...
            "AND char_length(provider_email_address)<=320 "
            "AND provider_email_private IS NOT NULL)",
            name=op.f("ck_external_link_challenge_provider_email"),
        ),
        sa.CheckConstraint(
            "(provider_code='google' AND apple_auth_grant_ref IS NULL) OR "
            "(provider_code='apple' AND apple_auth_grant_ref IS NOT NULL)",
            name=op.f("ck_external_link_challenge_provider_grant"),
        ),
        sa.CheckConstraint(
            "octet_length(continuation_verifier)=32",
            name=op.f("ck_external_link_challenge_continuation_verifier_length"),
        ),
        sa.CheckConstraint(
            "isfinite(created_at) AND isfinite(expires_at) "
            "AND expires_at>created_at "
            "AND expires_at<=created_at+interval '15 minutes'",
            name=op.f("ck_external_link_challenge_chronology"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_uuidv7"),
        ),
        sa.CheckConstraint(
            "provider_code IN ('google','apple')",
            name=op.f("ck_external_signup_challenge_provider_code"),
        ),
        sa.CheckConstraint(
            "(provider_code='google' AND issuer='https://accounts.google.com') OR "
            "(provider_code='apple' AND issuer='https://appleid.apple.com')",
            name=op.f("ck_external_signup_challenge_provider_issuer"),
        ),
        sa.CheckConstraint(
            "subject=btrim(subject) AND subject<>'' AND char_length(subject)<=255",
            name=op.f("ck_external_signup_challenge_subject"),
        ),
        sa.CheckConstraint(
            "(provider_email_address IS NULL AND provider_email_private IS NULL) OR "
            "(provider_email_address IS NOT NULL "
            ...
            "AND char_length(provider_email_address)<=320 "
            "AND provider_email_private IS NOT NULL)",
            name=op.f("ck_external_signup_challenge_provider_email"),
        ),
        sa.CheckConstraint(
            "(provider_code='google' AND apple_auth_grant_ref IS NULL) OR "
            "(provider_code='apple' AND apple_auth_grant_ref IS NOT NULL)",
            name=op.f("ck_external_signup_challenge_provider_grant"),
        ),
        sa.CheckConstraint(
            "octet_length(continuation_verifier)=32",
            name=op.f("ck_external_signup_challenge_continuation_verifier_length"),
        ),
        sa.CheckConstraint(
            "(email_address IS NULL AND email_comparison_key IS NULL) OR "
            "(email_address IS NOT NULL AND email_comparison_key IS NOT NULL)",
            name=op.f("ck_external_signup_challenge_email_pair"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_email_verification"),
        ),
        sa.CheckConstraint(
            "failed_verification_attempts BETWEEN 0 AND 5",
            name=op.f("ck_external_signup_challenge_failed_attempts"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_bootstrap_display_name"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_bootstrap_given_name"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_bootstrap_family_name"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_bootstrap_picture_url"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_external_signup_challenge_bootstrap_locale"),
        ),
        sa.CheckConstraint(
            "isfinite(created_at) AND isfinite(updated_at) "
            ...
            name=op.f("ck_external_signup_challenge_chronology"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_account_profile_bootstrap_source_provider_code"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_account_profile_bootstrap_source_provider_issuer"),
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_webauthn_account_user_handle_length"),
        ),
        sa.CheckConstraint("isfinite(created_at)", name=op.f("ck_webauthn_account_created_at")),
        schema=_SCHEMA,
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_passkey_credential_uuidv7"),
        ),
        sa.CheckConstraint(
            "octet_length(credential_id) BETWEEN 1 AND 1023",
            name=op.f("ck_passkey_credential_credential_id_length"),
        ),
        sa.CheckConstraint(
            "octet_length(credential_public_key) BETWEEN 1 AND 8192",
            name=op.f("ck_passkey_credential_public_key_length"),
        ),
        sa.CheckConstraint(
            "sign_count BETWEEN 0 AND 4294967295",
            name=op.f("ck_passkey_credential_sign_count"),
        ),
        sa.CheckConstraint(
            "cardinality(transports)<=8 AND array_position(transports,NULL) IS NULL",
            name=op.f("ck_passkey_credential_transports"),
        ),
        sa.CheckConstraint(
            "NOT backup_state OR backup_eligible",
            name=op.f("ck_passkey_credential_backup_state"),
        ),
        sa.CheckConstraint(
            "label=btrim(label) AND label<>'' AND char_length(label)<=100",
            name=op.f("ck_passkey_credential_label"),
        ),
        sa.CheckConstraint(
            "status_code IN ('active','revoked')",
            name=op.f("ck_passkey_credential_status_code"),
        ),
        sa.CheckConstraint(
            "isfinite(created_at) AND isfinite(updated_at) "
            ...
            name=op.f("ck_passkey_credential_chronology"),
        ),
        sa.CheckConstraint(
            "(status_code='active' AND revoked_at IS NULL "
            "AND revocation_reason_code IS NULL) OR "
            ...
            name=op.f("ck_passkey_credential_revocation"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_webauthn_challenge_session_binding"),
        ),
        sa.CheckConstraint(
            "isfinite(created_at) AND isfinite(expires_at) "
            "AND expires_at>created_at "
            "AND expires_at<=created_at+interval '5 minutes'",
            name=op.f("ck_webauthn_challenge_chronology"),
        ),
        sa.CheckConstraint(
            "claimed_at IS NULL OR (isfinite(claimed_at) "
            "AND claimed_at>=created_at AND claimed_at<=expires_at)",
            name=op.f("ck_webauthn_challenge_claimed_at"),
        ),
        schema=_SCHEMA,
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    for table_name in reversed(_M5_TABLES):
        op.execute(
            sa.text(f"REVOKE ALL PRIVILEGES ON TABLE {_SCHEMA}.{table_name} FROM {_RUNTIME}")
        )
