...
BEGIN
  ...
  IF FOUND THEN
    IF receipt.intent_fingerprint <> requested_fingerprint
       OR receipt.operation_kind <> 'create' THEN
...
BEGIN
  ...
  IF FOUND THEN
    IF receipt.intent_fingerprint <> requested_fingerprint
       OR receipt.operation_kind <> 'rename' THEN
      RAISE EXCEPTION USING ERRCODE='23505',
        CONSTRAINT='person_referent_operation_reused',
        MESSAGE='Person referent operation id reused';
    END IF;
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("self_person_ref","person_ref",name="pk_person_referent_catalog"),
        ...
        sa.CheckConstraint("btrim(display_label)<>'' AND char_length(display_label)<=100",
                           name="ck_person_referent_catalog_label"),
        sa.CheckConstraint("revision>=1",name="ck_person_referent_catalog_revision"),
        schema="dante",
    )
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("self_person_ref","operation_id",name="pk_person_referent_operation"),
        ...
        sa.CheckConstraint("operation_id=btrim(operation_id) AND operation_id<>'' "
                           ...
                           name="ck_person_referent_operation_id"),
        sa.CheckConstraint("intent_fingerprint ~ '^[0-9a-f]{64}$'",
                           name="ck_person_referent_operation_fingerprint"),
        sa.CheckConstraint("operation_kind IN ('create','rename')",
                           name="ck_person_referent_operation_kind"),
        sa.CheckConstraint("btrim(display_label)<>'' AND char_length(display_label)<=100",
                           name="ck_person_referent_operation_label"),
        sa.CheckConstraint("resulting_revision>=1",
                           name="ck_person_referent_operation_revision"),
        schema="dante",
    )
    _sql(_CREATE)
    _sql(_LIST)
