"""B10-A: canonicalize Actual capability function signatures.

Revision ID: 20260925_73
Revises: 20260925_72

B10-A initially used generic implementation overloads behind the exact
subject-family public functions. The DANTE Dictionary models one canonical
routine per PostgreSQL proname, so hidden overloads would break the exact
cross-representation contract. Inline the implementation into the family-aware
write/read functions and retire the two generic overloads.
"""
...
revision: str = "20260925_73"
down_revision: str | None = "20260925_72"
...
def upgrade() -> None:
    _sql(
        r"""
CREATE OR REPLACE FUNCTION dante.record_self_actual_realization(
...
"""
    ...
    _sql(
        r"""
CREATE OR REPLACE FUNCTION dante.get_self_subject_actual(
  requested_self_person_ref uuid,
  requested_subject_family text,
  requested_subject_native_ref uuid
) RETURNS TABLE(
...
"""
    ...
    _sql(
        "DROP FUNCTION dante.record_self_actual_realization("
        "uuid,text,text,uuid,uuid,uuid,uuid,boolean,text,timestamptz,timestamptz,uuid[],uuid[])"
    )
    _sql("DROP FUNCTION dante.get_self_subject_actual(uuid,uuid)")

    write_signature = (
        "dante.record_self_actual_realization("
        "uuid,text,text,text,uuid,uuid,uuid,uuid,boolean,text,timestamptz,timestamptz,uuid[],uuid[])"
    )
    read_signature = "dante.get_self_subject_actual(uuid,text,uuid)"
    for signature in (write_signature, read_signature):
...
def downgrade() -> None:
    raise RuntimeError(
        "B10-A canonical Actual signature repair downgrade is intentionally refused; "
