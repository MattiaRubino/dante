...
class AppleFlowService:
    ...
    async def begin_apple(
        ...
        *,
        ...
        """Persist one Apple transaction and return only its exact trusted authorize URL."""
    ...
    async def complete_apple(
        ...
        *,
        ...
        """Claim Apple state before single-use exchange, then converge on DANTE AuthSession truth."""
        ...
        except AppleProtocolError as exc:
        ...
        if transaction.purpose is ProviderPurpose.SIGN_IN:
            if existing is not None and existing.status_code == "active":
            ...
            grant_ref = await self._persist_pending_grant(
                evidence=evidence,
                refresh_token=tokens.refresh_token,
            )
            if existing is not None:
                return await self._link_required_for_bound_identity(
                    identity=existing,
                    evidence=evidence,
                    grant_ref=grant_ref,
                )
    ...
    async def set_provider_enrollment_email(
        ...
        *,
        ...
        try:
            ...
            if challenge.provider_code != "apple" or challenge.apple_auth_grant_ref is None:
    ...
    async def verify_provider_enrollment(
        ...
        *,
        ...
        """Consume Apple enrollment OTP and atomically create Account or link challenge."""
        await self._limiters.enrollment.consume(source_context, code="auth.provider_rate_limited")
        now = datetime.now(UTC)
        account_ref = uuid7()
        ...
        try:
            ...
            if (
                challenge.provider_code != "apple"
                or challenge.apple_auth_grant_ref is None
            ...
            grant = await database_session.scalar(
                ...
                .where(
                    AppleAuthGrantRow.apple_auth_grant_ref == snapshot.apple_auth_grant_ref,
                    AppleAuthGrantRow.issuer == snapshot.issuer,
                    AppleAuthGrantRow.subject == snapshot.subject,
                )
                .with_for_update()
            )
            if grant is None or grant.status_code != "pending" or grant.pending_expires_at is None:
                raise ProviderEnrollmentInvalidOrExpiredError()
            ...
            return await self._resolve_enrollment_race(snapshot=snapshot, constraint=self._constraint_name(exc))
        except SQLAlchemyError as exc:
            await self._safe_rollback(database_session)
            raise AuthServiceUnavailableError(retryable=True) from exc
        finally:
    ...
    async def revoke_identity_and_grant(
        ...
        *,
        ...
        """Disable Apple locally first, then revoke the encrypted refresh grant remotely."""
        ...
        try:
            ...
            if identity is None:
                raise ProviderProofInvalidError()
            await database_session.execute(select(func.dante.acquire_account_security_lock(identity.account_ref)))
            identity = await database_session.scalar(
                select(ExternalIdentityRow).where(
                    ExternalIdentityRow.external_identity_ref == external_identity_ref
                )
    ...
    async def reconcile_expired_pending_grants(self) -> int:
        """Move a bounded batch of abandoned pending grants through remote revoke reconciliation."""
        now = datetime.now(UTC)
        try:
                ...
                refs = list(
                    (
                        await database_session.scalars(
                            ...
                            .limit(_EXPIRED_RECONCILE_BATCH)
                        )
                    ).all()
                )
        except SQLAlchemyError as exc:
            raise AuthServiceUnavailableError(retryable=True) from exc
        completed = 0
        for grant_ref in refs:
            if await self._mark_pending_grant_for_revoke(grant_ref):
                try:
                    await self._revoke_grant_remote(grant_ref)
                except (ProviderUnavailableError, ProviderReconciliationPendingError):
                    continue
                completed += 1
        return completed
    ...
    async def _complete_authenticated_link(
        ...
        *,
        ...
        try:
            ...
            await database_session.close()
        if bound is None:
            raise AuthIntegrityError("Apple link completed without bound AuthSession")
        return self._issued_session(
            account_ref=bound.account_ref,
    ...
    async def _complete_reauthentication(
        ...
        *,
        ...
        try:
            ...
            await database_session.close()
        if bound is None:
            raise AuthIntegrityError("Apple reauthentication completed without bound AuthSession")
        return self._issued_session(
            account_ref=bound.account_ref,
            auth_session_ref=bound.auth_session_ref,
            authenticated_at=bound.authenticated_at,
    ...
    async def _link_required_for_bound_identity(
        ...
        *,
        ...
        target: EmailIdentityRow | None = None
        if identity.email_identity_ref is not None:
            target = await self._read_email_identity_by_ref(
                identity.email_identity_ref,
                identity.account_ref,
            )
        if target is None and evidence.email is not None:
    ...
    async def _persist_pending_grant(
        self,
        *,
        evidence: AppleIdentityEvidence,
        refresh_token: SecretStr,
        ...
        try:
            ...
            encrypted = self._grant_cipher.encrypt(
            ...
            if grant is None:
                ...
                grant.external_identity_ref = None
                ...
                grant.status_code = "pending"
                grant.updated_at = now
                grant.status_changed_at = now
                grant.pending_expires_at = now + _PENDING_GRANT_TTL
            ...
            await self._safe_rollback(database_session)
            existing = await self._read_grant(evidence.issuer, evidence.subject)
            if existing is None:
                raise AuthServiceUnavailableError(retryable=True) from exc
            return await self._replace_pending_grant(existing, evidence=evidence, refresh_token=refresh_token)
        except SQLAlchemyError as exc:
            await self._safe_rollback(database_session)
            raise AuthServiceUnavailableError(retryable=True) from exc
        finally:
    ...
    async def _replace_pending_grant(
        self,
        existing: AppleAuthGrantRow,
        *,
        evidence: AppleIdentityEvidence,
        refresh_token: SecretStr,
    ) -> UUID:
        ...
        try:
            encrypted = self._grant_cipher.encrypt(
                plaintext=refresh_token,
                grant_ref=existing.apple_auth_grant_ref,
                subject=evidence.subject,
                client_id=self._apple_client_id(),
            )
        except AppleGrantCryptoError as exc:
        ...
        try:
            async with self._session_factory() as database_session, database_session.begin():
                grant = await database_session.scalar(
                    select(AppleAuthGrantRow)
                    .where(
                        AppleAuthGrantRow.apple_auth_grant_ref == existing.apple_auth_grant_ref,
                        AppleAuthGrantRow.issuer == evidence.issuer,
                        AppleAuthGrantRow.subject == evidence.subject,
                    )
                    .with_for_update()
            ...
            raise AuthServiceUnavailableError(retryable=True) from exc
        return existing.apple_auth_grant_ref
    ...
    async def _claim_transaction(self, state: str) -> _ClaimedAppleTransaction:
        ...
        try:
            ...
            row.claimed_at = now
            result = _ClaimedAppleTransaction(
                purpose=purpose,
    ...
    async def _apply_revocation_event(self, event: AppleNotificationEvent) -> bool:
        ...
        try:
            async with self._session_factory() as database_session, database_session.begin():
                ...
                await database_session.execute(select(func.dante.acquire_account_security_lock(identity.account_ref)))
                identity.status_code = "revoked"
                identity.status_changed_at = max(now, identity.status_changed_at)
                identity.revoked_at = max(now, identity.created_at)
                identity.revocation_reason_code = reason
                grant = await database_session.scalar(
    ...
    async def _revoke_grant_remote(self, grant_ref: UUID) -> None:
        ...
        if (
            grant is None
            or grant.status_code != "revocation_pending"
            or grant.refresh_token_ciphertext is None
            or grant.refresh_token_nonce is None
            or grant.encryption_key_id is None
        ):
            return
        try:
            token = self._grant_cipher.decrypt(
            ...
            raise AuthIntegrityError("Apple pending revoke grant cannot be decrypted") from exc
        try:
        ...
        try:
            async with self._session_factory() as database_session, database_session.begin():
                ...
                if current is None:
                    return
                current.status_code = "revoked"
                current.updated_at = now
                current.status_changed_at = now
                current.pending_expires_at = None
                current.revoked_at = now
                current.refresh_token_ciphertext = None
                current.refresh_token_nonce = None
                current.encryption_key_id = None
        except SQLAlchemyError as exc:
            raise AuthServiceUnavailableError(retryable=True) from exc
        ...
        *,
        ...
        try:
            async with self._session_factory() as database_session, database_session.begin():
                ...
                return self._stage_link_challenge(
                    database_session,
                    snapshot=snapshot,
                    target_email=collision,
                    now=datetime.now(UTC),
    ...
    async def _lock_bound_session(
        ...
        *,
        ...
        if initial is None:
            raise ProviderTransactionInvalidOrExpiredError()
        await database_session.execute(select(func.dante.acquire_account_security_lock(initial.account_ref)))
        account = await database_session.scalar(
    ...
    async def _delete_stale_challenges(
        ...
        *,
        ...
        await database_session.execute(
            delete(ExternalLinkChallengeRow).where(
                ExternalLinkChallengeRow.issuer == evidence.issuer,
                ExternalLinkChallengeRow.subject == evidence.subject,
            )
        )
        await database_session.execute(
            delete(ExternalSignupChallengeRow).where(
    ...
    def _normalize_enrollment_email(self, email: str) -> NormalizedEmail:
        try:
    ...
    def _provider_otp_ttl(self) -> timedelta:
        return min(
            timedelta(seconds=self._settings.signup_verification_ttl_seconds),
    ...
    async def _read_email_identity_by_ref(
        ...
        try:
            async with self._session_factory() as session, session.begin():
                row: EmailIdentityRow | None = await session.scalar(
                    select(EmailIdentityRow).where(
                        EmailIdentityRow.email_identity_ref == email_ref,
                        EmailIdentityRow.account_ref == account_ref,
                    )
                )
                return row
    ...
    async def _read_grant(self, issuer: str, subject: str) -> AppleAuthGrantRow | None:
        try:
            async with self._session_factory() as session, session.begin():
