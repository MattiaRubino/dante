...
from dante.platform.config.auth_provider import AuthProviderSettings
...
def _normalize_base_url(value: str, *, name: str) -> str:
    ...
    authority = host_for_authority if port is None or port == default_port else f"{host_for_authority}:{port}"
...
class AuthSettings(BaseModel):
    ...
    provider: AuthProviderSettings = Field(default_factory=AuthProviderSettings)
    ...
    def validate_smtp_username(cls, value: str | None) -> str | None:
        return None if value is None else _trimmed_identity(value, name="smtp_username")
    ...
    def validate_secret_rings_and_transport(self) -> Self:
        ...
        csrf_bytes = _decode_canonical_secret(self.csrf_key.get_secret_value(), name="csrf_key")
        ...
        purpose_sets = [set(pepper_bytes.values()), set(otp_bytes.values()), {csrf_bytes}, set(apple_grant_bytes.values())]
    ...
    def _decode_ring(values: dict[str, SecretStr], *, name: str) -> dict[str, bytes]:
        ...
        for key_id, secret in values.items():
            if not key_id or key_id.strip() != key_id or any(character in key_id for character in "\r\n"):
                raise ValueError(f"{name} keys must be non-blank, trimmed and single-line")
            decoded[key_id] = _decode_canonical_secret(secret.get_secret_value(), name=f"{name}[{key_id!r}]")
        return decoded
