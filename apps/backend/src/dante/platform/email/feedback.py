"""Provider-feedback normalization and durable Email Platform persistence."""
...
class EmailSuppressionProjectionPort(Protocol):
    """Optional consumer projection invoked after durable platform suppression changes."""
    ...
    async def apply(
        ...
        *,
        ...
        """Project one active hard-bounce/complaint into consumer-owned state."""
        ...
...
class EmailFeedbackStore:
    """Persist provider evidence and current shared delivery-suppression state."""
    ...
    def __init__(
        ...
        *,
        ...
        self._suppression_projection = suppression_projection
    ...
    async def record(
        ...
        *,
        ...
        if attempt is None or attempt.provider_code != "ses":
            _LOGGER.warning(
                "email_platform.feedback_uncorrelated provider=ses event_type=%s",
        ...
        _LOGGER.info(
            "email_platform.feedback_recorded provider=ses event_type=%s "
            "hard_failure=%s intent_ref=%s",
            feedback.event_type_code,
    ...
    async def _apply_suppression(
        ...
        *,
        ...
        if self._suppression_projection is not None:
            await self._suppression_projection.apply(
                database_session,
                recipient_comparison_key=intent.recipient_comparison_key,
                observed_at=feedback.provider_occurred_at,
            )
