...
_CURRENT_REVISION = "20260925_68"
_CURRENT_TOPOLOGY = (156, 5, 106, 93, 301, 251, 426, 0, 0, 0)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    ...
    assert (len(tables), len(views), len(routines)) == (156, 5, 106)
    ...
    assert current["embedded_objects"] == {
        "triggers": 93,
        "physical_indexes": 301,
    }
    assert current["constraints"] == {
        "foreign_keys": 251,
        "check_constraints": 426,
    }
    assert len(MAPPED_TABLES) == len(Base.registry.mappers) == len(Base.metadata.tables) == 156
