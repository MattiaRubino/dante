...
_CURRENT_REVISION = "20260920_42"
_CURRENT_TOPOLOGY = (116, 5, 44, 90, 233, 153, 331, 0, 0, 0)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    ...
    assert (len(tables), len(views), len(routines)) == (116, 5, 44)
    ...
    assert current["embedded_objects"] == {
        "triggers": 90,
        "physical_indexes": 233,
    }
    assert current["constraints"] == {
        "foreign_keys": 153,
        "check_constraints": 331,
    }
    assert (
        len(MAPPED_TABLES)
