...
_CURRENT_REVISION = "20260917_29"
_CURRENT_TOPOLOGY = (101, 5, 31, 78, 198, 119, 297, 0, 0, 0)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    ...
    assert (len(tables), len(views), len(routines)) == (101, 5, 31)
    ...
    assert current["embedded_objects"] == {"triggers": 78, "physical_indexes": 198}
    assert current["constraints"] == {
        "foreign_keys": 119,
        "check_constraints": 297,
    }
    assert (
        len(MAPPED_TABLES)
