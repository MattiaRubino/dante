...
_CURRENT_REVISION = "20260925_66"
_CURRENT_TOPOLOGY = (153, 5, 99, 93, 298, 242, 419, 0, 0, 0)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    ...
    assert (len(tables), len(views), len(routines)) == (153, 5, 99)
    ...
    assert current["embedded_objects"] == {
        "triggers": 93,
        "physical_indexes": 298,
    }
    assert current["constraints"] == {
        "foreign_keys": 242,
        "check_constraints": 419,
    }
    assert len(MAPPED_TABLES) == len(Base.registry.mappers) == len(Base.metadata.tables) == 153
