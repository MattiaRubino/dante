...
def _entries(kind: str) -> dict[str, dict[str, Any]]:
    return {path.stem: json.loads(path.read_text(encoding="utf-8")) for path in sorted((_DICTIONARY_ROOT / kind).glob("*.json"))}


def _dictionary_identifiers(tables: dict[str, dict[str, Any]], routines: dict[str, dict[str, Any]]) -> tuple[set[str], set[str], set[str]]:
    indexes: set[str] = set(); constraints: set[str] = set(); triggers: set[str] = set()
    for name, entry in tables.items():
        ...
        primary_key = entry["structure"]["primary_key"]; assert primary_key is not None; constraints.add(str(primary_key["name"]))
        for group in ("unique_constraints", "check_constraints"):
            for constraint in entry["structure"][group]:
                identifier = str(constraint["name"]); assert identifier not in constraints; constraints.add(identifier)
        for foreign_key in entry["structure"]["foreign_keys"]:
            identifier = str(foreign_key["name"]); assert identifier not in constraints; constraints.add(identifier)
        for index in entry["structure"]["indexes"]:
            identifier = str(index["name"]); assert identifier not in indexes; indexes.add(identifier)
        for trigger in entry["structure"]["triggers"]:
            identifier = str(trigger["name"]); assert identifier not in triggers; triggers.add(identifier)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    tables = _entries("tables"); views = _entries("views"); routines = _entries("routines")
    expected_indexes, expected_constraints, expected_triggers = _dictionary_identifiers(tables, routines)
    scope = json.loads((_DICTIONARY_ROOT / "scope.json").read_text(encoding="utf-8"))
    with _admin(migrated_database) as connection:
        environment = connection.execute("SELECT current_setting('server_version_num'),current_setting('server_encoding'),current_setting('max_identifier_length')").fetchone()
        ...
        """).fetchone()
        live_tables = {str(r[0]) for r in connection.execute("SELECT tablename FROM pg_tables WHERE schemaname='dante' AND tablename<>'alembic_version'")}
        live_views = {str(r[0]) for r in connection.execute("SELECT viewname FROM pg_views WHERE schemaname='dante'")}
        live_routines = {str(r[0]) for r in connection.execute("SELECT p.proname FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='dante'")}
        live_constraints = {str(r[0]) for r in connection.execute("SELECT con.conname FROM pg_constraint con JOIN pg_class c ON c.oid=con.conrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='dante' AND c.relname<>'alembic_version' AND con.contype IN ('p','u','f','c')")}
        live_indexes = {str(r[0]) for r in connection.execute("SELECT indexname FROM pg_indexes WHERE schemaname='dante' AND tablename<>'alembic_version'")}
        live_triggers = {str(r[0]) for r in connection.execute("SELECT t.tgname FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='dante' AND NOT t.tgisinternal")}
        owners = {str(r[0]) for r in connection.execute("SELECT DISTINCT pg_get_userbyid(c.relowner) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='dante' AND c.relname<>'alembic_version'")}
        ...
        current_revision = connection.execute("SELECT version_num FROM dante.alembic_version").fetchone()
    assert environment == ("180006", "UTF8", "63"); assert topology == _CURRENT_TOPOLOGY; assert current_revision == (_CURRENT_REVISION,)
    assert (len(tables), len(views), len(routines)) == (69, 5, 15)
    assert live_tables == set(tables); assert live_views == set(views); assert live_routines == set(routines)
    assert live_constraints == expected_constraints; assert live_indexes == expected_indexes; assert live_triggers == expected_triggers
    assert owners == {"dante_owner"}; assert extensions["postgis"] == "3.6.4"; assert extensions["vector"] == "0.8.6"
    assert set(extensions) == {"postgis", "vector", "pg_trgm", "unaccent", "pg_stat_statements"}
    current = scope["current_materialization"]
    assert current["completed_stages"] == ["CP6-M01", "CP6-M02", "CP6-M03", "CP6-M04", "CP6-M05", "CP6-M06", "CP6-M07", "RECOVERY-CP06"]
    ...
    for name, entry in tables.items():
        ...
        assert isinstance(row.__table__, Table); assert row.__table__.name == name; assert row.__table__.schema == "dante"
    config = Config(toml_file=str(_REPO_ROOT / "apps" / "backend" / "pyproject.toml")); scripts = ScriptDirectory.from_config(config)
