...
_CURRENT_REVISION = "20260922_52"
_CURRENT_TOPOLOGY = (139, 5, 73, 92, 269, 209, 389, 0, 0, 0)
...
def test_current_database_cross_representation_is_exact(migrated_database: Any) -> None:
    ...
    assert current["embedded_objects"] == {
        "triggers": 90,
        "physical_indexes": 263,
    }
    assert current["constraints"] == {
        "foreign_keys": 201,
        "check_constraints": 381,
    }
    assert len(MAPPED_TABLES) == len(Base.registry.mappers) == len(Base.metadata.tables) == 135
