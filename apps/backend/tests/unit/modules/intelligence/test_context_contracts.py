...
def test_source_content_is_not_current_user_instruction() -> None:
    distinct_roles = {
        InstructionProvenance.SOURCE_CONTENT,
        InstructionProvenance.CURRENT_USER_INSTRUCTION,
        InstructionProvenance.DATA,
        InstructionProvenance.TRUSTED_SYSTEM_INSTRUCTION,
    }
    assert len(distinct_roles) == 4
