...
test.describe('Timeline B01 canonical Activity vertical', () => {
  test('creates one unplaced Activity, reads it in Planning Tray and preserves identity after reload', async ({
    browser,
  }, testInfo) => {
    ...
    try {
      ...
      await page
        ...
        .click();
      await expect(page.getByRole('radio', { name: 'Attività' })).toHaveAttribute(
        'aria-checked',
        'true',
      );
