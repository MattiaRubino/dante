...
function normalizeRevision(value: unknown): number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
    throw new Error(
      'World Focus composition config revision must be a non-negative safe integer',
    );
  }
  return value;
}
...
function normalizeVisibility(
  value: unknown,
): WorldFocusCompositionConfigVisibility {
  if (value !== 'visible' && value !== 'hidden') {
    throw new Error(
      'World Focus composition config visibility must be visible or hidden',
    );
  }
  return value;
}
...
function normalizePinned(value: unknown): boolean {
  if (typeof value !== 'boolean') {
    throw new Error(
      'World Focus composition config pinned state must be boolean',
    );
  }
  return value;
}
...
function normalizeProminenceOverride(
  ...
  if (value !== null && value !== 'lead') {
    throw new Error(
      'World Focus composition prominence override must be lead or null',
    );
  }
  return value;
}
...
export function createWorldFocusCompositionConfig(
  ...
  if (input.schemaVersion !== WORLD_FOCUS_COMPOSITION_CONFIG_SCHEMA_VERSION) {
    throw new Error(
      'World Focus composition config schema version is not current',
    );
  }
  ...
  if (worldId === undefined) {
    throw new Error(
      'World Focus composition config World id must not be empty',
    );
  }
