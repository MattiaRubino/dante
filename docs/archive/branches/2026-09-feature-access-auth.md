...
- Forward Recovery↔Email hardening PR: `#55`
- Forward hardening merge commit: `c67a18c24a6cf22b003ffd2c14243af53fec5077`
- PostgreSQL: 18.6
...
A post-merge red-team audit identified a narrower cross-subsystem acceptance gap: the CP07 harness did **not** directly exercise the shared Email Platform's `quarantine_after_restore()` path before Email workers resume. The Email quarantine implementation and its direct PostgreSQL test existed, but the whole recovery/reopen rehearsal had not proved their ordering as one end-to-end restore scenario.

Therefore the historical CP07 result remains valid for the PostgreSQL/database-local and MaterialState scope it actually executed, and this archive must not be used to pretend that application Email traffic reopen after PITR was already end-to-end proven by CP07.

## Forward closure after the historical Access integration

That post-merge gap was closed **forward**, outside the Access branch history, through PR #55. The historical record above is intentionally preserved rather than rewritten.

On proof head `1a5a7f1fbbdc1e5723d58fa90721a8693cce49e9`, mandatory CI passed and the real LOCAL CP08 rehearsal directly proved PITR into resurrected sendable Email state, worker isolation, fail-closed quarantine of all restored sendable intents, `in_progress → ambiguous`, sensitive-material wipe, idempotent second reconciliation, zero claimable Email work and application/Email reopen PASS. PR #55 then merged at `c67a18c24a6cf22b003ffd2c14243af53fec5077`.

This forward closure is current Recovery/Email history, not retroactive Access-branch evidence.
