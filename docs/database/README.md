> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-25 / DB-U26 SECURITY HARDENING CLOSED**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–18 as one canonical authority**. Part 18 closes **DB-U26 — Database Security Execution Hardening** after an independent read-only audit against the real backend foundation plus current PostgreSQL 18 / psycopg 3.3.4 behavior. It freezes parameter/identifier construction rules, zero generic dynamic SQL for the 14 integrity routines, trusted `search_path = pg_catalog,dante,pg_temp`, explicit `pg_temp`-last object-hijack defense, SCRAM-safe password handling without cleartext password-bearing SQL, exact runtime/migrator/owner identity, exact DANTE role-membership topology, owner no-password posture, and the DB security negative-proof contract.  
> Frozen object counts remain **68 tables / 5 views / 14 routines / 75 triggers / 95 indexes / 68 FKs / 120 CHECKs**. **DB-U26 is CLOSED as blueprint/security-design; direct implementation proof remains staged to CP6-04/05.** The exact next CP6-03 operation is the **fresh independent Parts 1–18 tombstone + security replay from zero**. Gate 03 is NOT yet earned; CP6-04 remains NOT STARTED / NOT AUTHORIZED; protected-`main` realignment remains intentionally deferred. This overlay supersedes only older CURRENT/layout prose below that stops at Part 17 or describes DB-U26 as proposed/open. Historical layout/evidence remains retained.  

> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / SECOND TOMBSTONE REPAIR COMPLETE**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–17 as one canonical authority**. Part 17 closes **DB-U25 — Second Tombstone Repair** and repairs the five real B findings found by the first post-hardening replay: exact Recurrence selector/phase/range determinism, non-quota generation-coordinate duplication, five NULL-unsafe CHECK expressions, exact governing-Recurrence membership for materialized Occurrences, and one-way current-history lifecycle/column-scoped INSERT. Frozen object counts remain **68 tables / 5 views / 14 routines / 75 triggers / 95 indexes / 68 FKs / 120 CHECKs**.  
> The mandatory **SECOND FULL TOMBSTONE AUDIT FROM ZERO must now restart over Parts 1–17**; the pre-repair audit is not a PASS. Gate 03 is NOT yet earned and CP6-04 remains NOT STARTED / NOT AUTHORIZED. Protected-`main` realignment remains intentionally deferred. This overlay supersedes only older CURRENT/layout prose below that stops at Part 16 or treats the pre-repair second audit as complete; historical layout/evidence remains retained.  

> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / PRE-GATE03 HARDENING COMPLETE**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–16 as one canonical authority**. Part 16 closes **DB-U24 — Final Gate-03 Implementation-Determinism Hardening** and freezes the **Direct PostgreSQL Proof / Test Plan**. Frozen baseline remains 68 tables / 5 views / 14 routines / 75 triggers / 95 indexes / 68 FKs, with the exact declarative floor now including **120 CHECK constraints**. The Dictionary v1 readiness contract is hardened accordingly. **GLOBAL DB-U OPEN = 0**. The exact next CP6-03 operation is the **SECOND FULL TOMBSTONE AUDIT FROM ZERO**.  
> Gate 03 is NOT yet earned and CP6-04 remains NOT STARTED / NOT AUTHORIZED. Protected-`main` realignment remains intentionally deferred. This overlay supersedes only older CURRENT/layout prose below that stops at Part 15 or says the proof/test plan is still pending; historical layout/evidence remains retained.  

> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / DATABASE DICTIONARY READY**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–15 as one canonical authority**. `dante-postgresql-database-part-15.md` freezes **Database Dictionary Readiness** and `docs/database/dictionary/` now contains the versioned JSON/JSON-Schema foundation and readiness-only scope contract. Final target is **68 table + 5 view + 14 routine = 87 standalone entries**, with 75 trigger attachments and 95 physical indexes embedded under owning table entries. The exact next CP6-03 block is **DIRECT POSTGRESQL PROOF / TEST PLAN**.  
> This overlay supersedes only older CURRENT/layout prose below that stops at Part 14 or says Database Dictionary readiness is still pending/next. Object-specific entries remain intentionally absent until CP6-04 materializes the corresponding real objects. Historical layout/evidence remains retained; CP6-04 remains NOT STARTED / NOT AUTHORIZED and protected-`main` realignment remains deferred.  
...
> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / DB-U21 CLOSED**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–12 as one canonical authority**. `dante-postgresql-database-part-12.md` closes **DB-U21 — Exact Object-Level PostgreSQL Privilege Matrix**. The final global DB-U register is now **0 open items**. The exact next CP6-03 block is **MIGRATION / MATERIALIZATION DAG**. DB-U21 freezes the 68-table/5-view runtime ACL matrix, migration-owned exact business ACL doctrine, removal of CP3 blanket business grants during CP6-04, PUBLIC/schema hardening and the runtime-compatible advisory-serialization hardening for immutable owners.  
> This overlay supersedes only older CURRENT/layout prose below that stops at Part 11 or says DB-U21 is still open/next. Historical layout/evidence remains deliberately retained. CP6-04 remains NOT STARTED / NOT AUTHORIZED, and protected-`main` realignment remains intentionally deferred to a later separate gate.  

> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / DB-U15 CLOSED**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–11 as one canonical authority**. `dante-postgresql-database-part-11.md` closes **DB-U15 — Final Structural / Query Index Matrix**. Final index count is **95 DANTE-owned indexes**: the previously frozen 78-index structural floor plus 16 justified referencing-side FK/structural indexes and one quota-period query/concurrency index. The surviving global DB-U set is now exactly **DB-U21 only**; the exact next CP6-03 block is **DB-U21 — exact object-level PostgreSQL privilege matrix**.  
> This overlay supersedes only older CURRENT/layout prose below that stops at Part 10, says DB-U15 is still open/next, or treats 78 as the final index count. Historical layout/evidence remains deliberately retained. CP6-04 remains NOT STARTED / NOT AUTHORIZED, and protected-`main` realignment remains intentionally deferred to a later separate gate.  

> **CURRENT DATABASE REFERENCE LAYOUT — 2026-08-24 / DB-U08 CLOSED**  
> The human-readable DANTE Database Architecture & Reference is now **Parts 1–10 as one canonical authority**. `dante-postgresql-database-part-10.md` closes **DB-U08 — Final PostgreSQL Object Naming** and freezes the repository-wide SQL naming standard, exact 68 PK names, 68 FK naming outcomes (17 direct convention names + 51 explicit overlength aliases), the current 78-index floor names, 14 integrity-routine names and 75 trigger names. The surviving global open set is now exactly `DB-U15 / DB-U21`; the exact next CP6-03 block is **DB-U15 — final structural/query index matrix**.  
> This overlay supersedes only older CURRENT/layout prose below that stops at Part 9 or says DB-U08 is still open/next. Historical layout/evidence sections below remain deliberately retained. CP6-04 remains NOT STARTED / NOT AUTHORIZED, and protected-`main` realignment remains intentionally deferred to a later separate gate.  
