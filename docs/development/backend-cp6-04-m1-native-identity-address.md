...
The direct test lane also deliberately reintroduces one legacy broad
...
Alembic now imports the registered mapping package before assigning
`target_metadata`.

A fail-fast registration check requires the registered mapped tables and
`Base.metadata` tables to match exactly.

The canonical migration chain is therefore:
...
The M1 PostgreSQL lane is expected to prove:
...
At the time this candidate is authored:
...
No previous P0 or CP3 test result is reused as M1 proof.

## 11. Explicit exclusions

M1 does not create or activate:
...
The next mandatory action after the repository write is fresh direct execution
against the disposable PostgreSQL 18.6 acceptance boundary. M2 remains blocked
until M1 earns a real direct PASS.
