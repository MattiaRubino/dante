...
- Current execution state: **FM-06A DEPENDENCY ARCHITECTURE PASS / FM-06 IN PROGRESS**
...
`package.json` declares Node `24.19.0`, pnpm `11.22.0`, exact root engineering devDependency pins, the dependency-cruiser architecture command, and the standard root scripts. `.node-version` declares Node `24.19.0`. `apps/web/package.json` and `apps/mobile/package.json` declare their exact application dependency baselines. `packages/design-tokens/package.json` declares the shared token compiler toolchain. `packages/i18n/package.json` declares the framework-free shared i18n core. `packages/time/package.json` declares the framework-free shared Temporal core and `temporal-polyfill 1.0.4`. The shared lockfile is committed and was generated by pnpm from the real WSL installation.
...
`pnpm architecture:check` is now a real local gate backed by `dependency-cruiser.config.mjs`. It checks the current DANTE-owned Web/Mobile/shared-package graph rather than recursively validating third-party package internals.
...
The runtime/root-tooling/Web/minimal-Mobile/shared-design-token/shared-i18n/shared-time and current dependency-architecture portion is directly proven. FM-05 shared-package materialization is complete; the full materialization target is not PASS until the remaining FM-06 enforcement/test/generation slices and FM-07 clean-baseline obligations are directly exercised.
...
Proceed to **FM-06B generated-source drift enforcement**.

FM-06A is directly validated and closed at its stated scope. FM-06B should now enforce deterministic drift for already-real committed generated sources, especially the design-token outputs and TanStack Router route tree. Do not mix unit-test baseline, Playwright, Mobile bundle smoke or GitHub Actions into this slice.
