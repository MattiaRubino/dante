...
Conversation message and Insight remain different presentation/semantic objects. Validated Insight may use its own registered surface.
...
Consequential confirmation uses governed blocking semantics; D6 still has no real backend effect integration.
...
```text
M4 complete
-> structural visual check only: geometry, hierarchy, obvious interaction mistakes

M5 contrasting complete Worlds
-> serious product/visual review using real variation

M6
-> implement/refine visual system, responsive detail, motion, a11y, performance and final manual validation
```

This prevents polishing transient layouts before DANTE and cross-World content pressure exist.
...
After M7, real verticals connect:
