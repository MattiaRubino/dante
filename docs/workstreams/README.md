...
- [`pre-physical-coherence.md`](pre-physical-coherence.md) — **DEFINITIVE CLOSED / FINAL QA PASS / integrated into `main` via PR #13 / POST-MERGE VERIFIED** after Phase 0–12 plus the independent total Pre-Physical audit. Former branch `chore/pre-physical-coherence` was merged and auto-deleted. Physical/backend implementation remains separately unauthorized.
...
- [`backend-foundation.md`](backend-foundation.md) — **NOT STARTED / DEFERRED**. Pre-Physical integration is satisfied, but Backend still requires a separately authorized/accepted Physical Model plus all applicable current security/runtime/integration/API prerequisites before implementation may start. Do not create `feature/backend-foundation` or execute backend/schema/API instructions yet.
- Physical Model — **READY FOR SEPARATE AUTHORIZATION, NOT STARTED / NOT AUTHORIZED**. Phase 10 provides its benchmark method, not its implementation or result. A separate user authorization and fresh workstream/write gate from current `main` are required before Physical work begins.

Completed/historical handoffs remain useful evidence, but current model/architecture authority and current `main` determine execution truth.
